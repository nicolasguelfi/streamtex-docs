import streamlit as st
import streamtex as stx
from streamtex import st_book, TOCConfig, NumberingMode, MarkerConfig, BannerConfig, PdfConfig, ExportConfig, ExportMode, ViewMode
from pathlib import Path

from custom.styles import Styles as s
from custom.themes import dark
import streamtex.styles as sts
import blocks

# Configure static sources
stx.set_static_sources([str(Path(__file__).parent / "static")])

# Page configuration
st.set_page_config(
    page_title="My Project",
    layout="wide",
    initial_sidebar_state="expanded",
)

# Inject dark theme
sts.theme = dark

# Table of Contents
toc = TOCConfig(
    numbering=NumberingMode.SIDEBAR_ONLY,
    toc_position=None,
    title_style=s.project.titles.main_title + s.center_txt + s.text.wrap.nowrap,
    content_style=s.large + s.text.colors.reset,
    sidebar_max_level=2,
    search=True,
)

# Marker navigation (PageUp/PageDown)
marker_config = MarkerConfig(
    auto_marker_on_toc=1,
    next_keys=["PageDown"],
    prev_keys=["PageUp"],
)

# Orchestrate blocks
st_book(
    [
        blocks.bck_welcome,
        blocks.bck_text_and_styles,
        blocks.bck_containers_and_spacing,
        blocks.bck_grids,
        blocks.bck_lists,
        blocks.bck_images,
        blocks.bck_code_blocks,
        blocks.bck_overlays_and_includes,
        blocks.bck_interactivity,
    ],
    toc_config=toc,
    marker_config=marker_config,
    paginate=True,
    banner=BannerConfig.full(),
    inspector=stx.InspectorConfig(enabled=True),
    pdf_config=PdfConfig(format="A4", landscape=True),
    view_modes=[ViewMode.PAGINATED, ViewMode.CONTINUOUS],
    # Auto-export to disk (disabled by default — change NEVER to ALWAYS to enable)
    exports=[
        ExportConfig(
            format="html",
            mode=ExportMode.NEVER,
            output_dir="./exports",
            filename="my-project",
            timestamp=True,
        ),
        ExportConfig(
            format="pdf",
            mode=ExportMode.NEVER,
            output_dir="./exports",
            filename="my-project",
            timestamp=True,
            pdf=PdfConfig(format="A4", landscape=True),
        ),
    ],
)
