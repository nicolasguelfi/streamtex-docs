...
Required for workspace management (`stx update` clones repos).
...
```
/stx-block:init Docker introduction course, 10 slides, dark theme
```
...
Generated by `stx install`. Example (standard preset):
...
# Update options
stx update --skip-sync       # Skip uv sync step
stx update --skip-profiles   # Skip Claude profile update
stx update --dry-run         # Show steps without executing
stx update --repair          # Run repair checks (broken venv, missing __init__.py)
...
└── .claude/
    ...
    ├── commands/              # Slash commands (/stx-block:*, /stx-ce:*, /stx-issue:*, etc.)
...
```bash
stx install --preset user    # Minimum preset for Claude profiles
stx update                   # Clone the streamtex-claude repo
stx claude install project .
```
